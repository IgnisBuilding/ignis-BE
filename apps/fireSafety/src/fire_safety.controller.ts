...
export class FireSafetyController {
  constructor(private readonly fireSafetyService: FireSafetyService, private readonly dataSource: DataSource) {}
  ...
  broadcastHazard(@Body() hazard: any) {
    ...
    try {
      ...
      const io = globalAny.__io || (globalAny.__appInstance && globalAny.__appInstance.get ? globalAny.__appInstance.get('io') : null) || globalAny.io || null;
    ...
    try {
      ...
      this.fireSafetyService.rebuildAllRoutes(false).then(res => {
        console.log('RebuildAllRoutes completed after hazard:', res);
        try {
          const globalAny: any = global as any;
          const io = globalAny.__io || (globalAny.__appInstance && globalAny.__appInstance.get ? globalAny.__appInstance.get('io') : null) || globalAny.io || null;
          if (io && typeof io.emit === 'function') io.emit('evacuationRoutes.rebuilt', res);
        } catch (e) {
  ...
  const res = await this.dataSource.query(query);
    return res && res[0] ? res[0].geojson : { type: 'FeatureCollection', features: [] };
  }
  ...
  async getBuildingDetailsGeoJSON() {
    // Combine exits and evacuation routes into a single FeatureCollection
    const query = `
      SELECT json_build_object('type','FeatureCollection','features', COALESCE(json_agg(f), '[]'::json)) AS geojson FROM (
        ...
        UNION ALL
        SELECT json_build_object('type','Feature','geometry', ST_AsGeoJSON(ST_Transform(er.path,4326))::json,'properties', json_build_object('id', er.id, 'feature_type','evacuation_route')) AS f FROM evacuation_route er
      ) t;
    `;
  const res = await this.dataSource.query(query);
    return res && res[0] ? res[0].geojson : { type: 'FeatureCollection', features: [] };
  ...
  async getSensors() {
    ...
    try {
      ...
      console.warn('getSensors query failed, returning empty FeatureCollection', e && e.message ? e.message : e);
  ...
  async getApartments() {
    try {
      ...
      console.warn('getApartments query failed, returning empty array', e && e.message ? e.message : e);
      return [];
    }
  }
  ...
  async getRooms() {
    const query = `
      SELECT json_build_object('type','FeatureCollection','features', COALESCE(json_agg(json_build_object('type','Feature','geometry', ST_AsGeoJSON(ST_Transform(geometry,4326))::json,'properties', json_build_object('id', id, 'name', name, 'room_type', type))), '[]'::json)) AS geojson
      FROM room;
    `;
    const res = await this.dataSource.query(query);
    return res && res[0] ? res[0].geojson : { type: 'FeatureCollection', features: [] };
  ...
  if (blocked && blocked.length > 0) {
      ...
      if (blockedNodes.includes(startNodeId) && blockedNodes.includes(endNodeId)) {
        return { error: true, message: 'Both start and end nodes are affected by active hazards; no safe route can be computed.' };
      }
      if (blockedNodes.includes(startNodeId)) {
        return { error: true, message: 'Start node is affected by an active hazard; cannot compute a safe route.' };
      }
      if (blockedNodes.includes(endNodeId)) {
        ...
        if (!safeExits || safeExits.length === 0) {
          return { error: true, message: 'End node is affected by an active hazard and no safe exits are available.' };
        }
        ...
        for (const ex of safeExits) {
          const exitId = ex.id;
          const pathWkt = await this.fireSafetyService.getPathWktIfExists(startNodeId, exitId, false);
          if (!pathWkt) continue;
          // compute length
          const lenRes = await this.dataSource.query(`SELECT ST_Length(ST_GeomFromText($1,3857)) AS len`, [pathWkt]);
        ...
        if (!best) {
          return { error: true, message: 'End node is affected by an active hazard and no alternative hazard-free routes could be computed.' };
        }
        ...
        return { alternative: true, targetNodeId: best.exitId, geojson: fakeInsert && fakeInsert[0] ? fakeInsert[0].geojson : null };
      }
    }
    ...
    if (stored && stored[0] && stored[0].id) {
      ...
      if (hazardIntersect && hazardIntersect.length > 0) {
        ...
        try {
          const newGeo = await this.fireSafetyService.computeAndSavePath(createRouteDto);
          // delete old route record
          await this.dataSource.query('DELETE FROM evacuation_route WHERE id = $1', [routeId]);
          return newGeo;
        } catch (err) {
          ...
          const storedGeo = await this.fireSafetyService.getRouteAsGeoJSON(routeId);
          return { warning: true, message: 'Stored route intersects active hazards', geojson: storedGeo };
        }
      }
  ...
  async debugRoute(@Body() dto: CreateRouteDto) {
    ...
    `;
    const pgrRows = await this.dataSource.query(pgrSql, [startNodeId, endNodeId]);

    // Collect any edge ids referenced (exclude -1)
    const edgeIds = (pgrRows || []).filter(r => r && r.edge && r.edge !== -1).map(r => r.edge);
    ...
    const hazards = await this.dataSource.query(`SELECT id, node_id, type, severity, status FROM hazards WHERE status <> 'CLEARED'`);

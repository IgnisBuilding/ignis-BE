...
async function bootstrap() {
  ...
  app.enableCors({ origin: process.env.ALLOWED_ORIGIN || 'http://localhost:4000', credentials: true });
  ...
  (global as any).__io = io;
    ...
    try {
      ...
      if (fireService && typeof fireService.rebuildAllRoutes === 'function') {
        console.log('Triggering initial rebuildAllRoutes (background, ignoring hazards)');
        // Precompute initial routes ignoring hazards so the evacuation table is populated
        fireService.rebuildAllRoutes(true).then(r => console.log('Initial rebuildAllRoutes done', r)).catch(e => console.warn('Initial rebuildAllRoutes failed', e));
      }
    } catch (e) {
